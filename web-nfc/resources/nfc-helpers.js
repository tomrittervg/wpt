...
function testNFCReaderOptions(message, readOptions, unacceptableReadOptions, desc) {
  nfc_test(async (t, mockNFC) => {
    const reader1 = new NFCReader(unacceptableReadOptions);
    const reader2 = new NFCReader(readOptions);

    // Sets acceptable NDEFCompatibility
    if (readOptions.compatibility) {
      mockNFC.setCompatibility(readOptions.compatibility);
    }
